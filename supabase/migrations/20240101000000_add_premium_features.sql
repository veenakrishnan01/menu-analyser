...
CREATE TABLE IF NOT EXISTS public.generated_offers (
  ...
  estimated_redemption_rate DECIMAL(5,2),
...
CREATE INDEX IF NOT EXISTS idx_generated_offers_user_id ON public.generated_offers(user_id);
CREATE INDEX IF NOT EXISTS idx_generated_offers_status ON public.generated_offers(status);
...
CREATE INDEX IF NOT EXISTS idx_premium_usage_user_feature ON public.premium_feature_usage(user_id, feature_type);
...
DROP TRIGGER IF EXISTS handle_updated_at_generated_offers ON public.generated_offers;
CREATE TRIGGER handle_updated_at_generated_offers
  BEFORE UPDATE ON public.generated_offers
  FOR EACH ROW EXECUTE PROCEDURE public.handle_updated_at();

DROP TRIGGER IF EXISTS handle_updated_at_ad_campaigns ON public.ad_campaigns;
CREATE TRIGGER handle_updated_at_ad_campaigns
  BEFORE UPDATE ON public.ad_campaigns
  FOR EACH ROW EXECUTE PROCEDURE public.handle_updated_at();

DROP TRIGGER IF EXISTS handle_updated_at_premium_feature_usage ON public.premium_feature_usage;
...
INSERT INTO public.subscription_tiers (name, price_monthly, price_yearly, features, max_offers_per_month, max_campaigns_per_month) 
VALUES
